...
class GRU4Rec(nn.Module):
    def __init__(self, args):
        ...
        self.item_embeddings = nn.Embedding(args.item_size, self.embedding_size, padding_idx=0)
...
class SRGNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        # define layers and loss
        # item embedding
        self.item_embeddings = nn.Embedding(self.n_items, self.embedding_size, padding_idx=0)
        # define layers and loss
    ...
    def finetune(self, item_seq):
        ...
        if self.args.model_name == 'Ori-SRGNN':
            ...
            # our change version
            hidden = torch.gather(hidden, dim=1, index=alias_inputs)            
            ht = hidden[torch.arange(mask.shape[0]).long(), torch.sum(mask, 1) - 1]  # batch_size x latent_size
            q1 = self.linear_one(ht).view(ht.shape[0], 1, ht.shape[1])  # batch_size x 1 x latent_size
            q2 = self.linear_two(hidden)  # batch_size x seq_length x latent_size
            alpha = self.linear_three(torch.sigmoid(q1 + q2))

            a = alpha * hidden * mask.view(mask.size(0), -1, 1).float()
            ht = ht.unsqueeze(1).repeat(1,50,1)
            a = self.linear_transform(torch.cat([a, ht], 2))
            return a
